...
function PublicFinancialServices() {
  ...
  const [files, setFiles] = useState({ aadhaarCard: null, panCard: null, addressProof: null, incomeProof: null, bankPassbook: null, photo: null });
  ...
  const handleSubmit = (e) => {
    ...
    navigate("/payment", { state: { serviceName: "Public Financial Services", applicantName: formData.fullName, mobile: formData.mobile, Amount: 400, type: "PFS", formData, documents: { aadhaarCard: files.aadhaarCard?.file, panCard: files.panCard?.file, addressProof: files.addressProof?.file, incomeProof: files.incomeProof?.file, bankPassbook: files.bankPassbook?.file, photo: files.photo?.file } } });
  };

  return (
    <div className="min-h-screen bg-[#f8faff] font-sans text-[#1e293b]">
      <section className="relative w-full h-[250px] sm:h-[350px] md:h-[500px] flex items-center">
        <div className="absolute inset-0"><img src={PanHero} alt="Public Financial Services" className="w-full h-full object-cover" /></div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full">
          <div className="w-full md:w-1/2 space-y-3 sm:space-y-6 text-white">
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-bold">Public Financial Services</h1>
            <p className="text-sm sm:text-lg md:text-xl text-gray-200">Assistance for various government financial schemes and public welfare services.</p>
            <a href="#pfs-form"><button className="bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-black px-5 sm:px-8 py-2 sm:py-3 rounded-xl font-bold text-sm sm:text-base shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">Apply Now</button></a>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-8 sm:py-12 px-3 sm:px-6 md:px-8">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white border-4 border-green-700 rounded-3xl p-4 sm:p-8 md:p-12 shadow-xl">
            ...
            <h3 className="text-base sm:text-2xl font-bold text-green-600 text-center mb-4 sm:mb-8 border-b-4 border-green-700 pb-3">Required Documents for Public Financial Services</h3>
            <div className="space-y-3 text-sm sm:text-lg">
              {[["आधार कार्ड", "Aadhaar Card"], ["पॅन कार्ड", "PAN Card"], ["राहण्याचा पुरावा", "Address Proof"], ["उत्पन्नाचा पुरावा", "Income Proof"], ["बँक पासबुक", "Bank Passbook Copy"], ["पासपोर्ट साईज फोटो", "Passport Size Photo"]].map((item, i) => (
      ...
      <section id="pfs-form" className="py-8 sm:py-12 px-3 sm:px-6 md:px-8 bg-[#f8faff]">
        <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden mb-10 sm:mb-20">
          <div className="bg-gradient-to-r from-blue-900 to-blue-700 px-4 sm:px-10 py-5 sm:py-8">
            <h2 className="text-lg sm:text-2xl md:text-3xl font-bold text-white text-center">Public Financial Services Application Form</h2>
