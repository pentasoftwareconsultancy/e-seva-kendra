...
function EShramForm() {
  ...
  const [files, setFiles] = useState({ aadhaar: null, bankPassbook: null, photo: null });
  ...
  const handleSubmit = (e) => {
    ...
    if (!files.aadhaar || !files.bankPassbook || !files.photo) { alert("Please upload all required documents"); return; }
    navigate("/payment", { state: { serviceName: "E-Shram Card", applicantName: formData.fullName, mobile: formData.mobile, Amount: 250, formData, documents: { aadhaar: files.aadhaar?.file, bankPassbook: files.bankPassbook?.file, photo: files.photo?.file } } });
  };

  return (
    <div className="min-h-screen bg-[#f8faff] font-sans text-[#1e293b]">
      <section className="relative w-full h-[250px] sm:h-[350px] md:h-[500px] flex items-center">
        <div className="absolute inset-0"><img src={PanHero} alt="E-Shram Card" className="w-full h-full object-cover object-center" /></div>
        <div className="absolute inset-0 bg-gradient-to-r from-[#0b2c6d]/95 via-[#143f8f]/80 to-transparent"></div>
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full">
          <div className="w-full md:w-1/2 space-y-3 sm:space-y-6 text-white">
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-bold">E-Shram Card Registration</h1>
            <p className="text-sm sm:text-lg md:text-xl text-gray-200">Register for Government E-Shram Card and get social security benefits.</p>
            <a href="#eshram-form"><button className="bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-black px-5 sm:px-8 py-2 sm:py-3 rounded-xl font-bold text-sm sm:text-base shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">Apply Now</button></a>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white border-4 border-green-700 rounded-3xl p-4 sm:p-8 md:p-12 shadow-xl">
            ...
            <div className="space-y-3 text-sm sm:text-lg">
              {[["आधार कार्ड", "Aadhaar Card"], ["मोबाईल नंबर (आधारशी लिंक)", "Mobile Number (linked with Aadhaar)"], ["बँक पासबुक", "Bank Passbook"], ["पासपोर्ट साइज फोटो", "Passport Size Photo"], ["रेशन कार्ड / निवासी पुरावा", "Ration Card / Address Proof"]].map((item, i) => (
      ...
      <section id="eshram-form" className="py-8 sm:py-12 px-3 sm:px-6 md:px-8 bg-[#f8faff]">
        <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden mb-10 sm:mb-20">
          <div className="bg-gradient-to-r from-blue-900 to-blue-700 px-4 sm:px-10 py-5 sm:py-8">
            <h2 className="text-lg sm:text-2xl md:text-3xl font-bold text-white text-center">E-Shram Card Application Form</h2>
