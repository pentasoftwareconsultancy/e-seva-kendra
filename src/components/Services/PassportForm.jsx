...
function PassportForm() {
  ...
  const [files, setFiles] = useState({ pan: null, aadhaar: null, photo: null, lightBill: null });
  ...
  const handleSubmit = (e) => {
    ...
    if (!files.pan || !files.aadhaar || !files.photo || !files.lightBill) { alert("Please upload all required documents"); return; }
    navigate("/payment", { state: { serviceName: "Passport Service", applicantName: formData.fullName, mobile: formData.mobile, Amount: 2500, formData, documents: { pan: files.pan?.file, aadhaar: files.aadhaar?.file, photo: files.photo?.file, lightBill: files.lightBill?.file } } });
  };

  return (
    <div className="min-h-screen bg-[#f8faff] font-sans text-[#1e293b]">
      <section className="relative w-full h-[250px] sm:h-[350px] md:h-[500px] flex items-center">
        <div className="absolute inset-0"><img src={Panhero} alt="Passport Hero" className="w-full h-full object-cover object-[20%_center]" /></div>
        <div className="absolute inset-0 bg-gradient-to-r from-[#0b2c6d]/95 via-[#143f8f]/80 to-transparent"></div>
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 w-full">
          <div className="w-full md:w-1/2 space-y-3 sm:space-y-6 text-white">
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-bold">Passport Services</h1>
            <p className="text-sm sm:text-lg md:text-xl text-gray-200">We provide fast and reliable assistance for Passport application and renewal services.</p>
            <a href="#passport-form"><button className="bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-black px-5 sm:px-8 py-2 sm:py-3 rounded-xl font-bold text-sm sm:text-base shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">Apply Now</button></a>
          </div>
        </div>
      </section>
      ...
      <section id="passport-form" className="py-8 sm:py-12 px-3 sm:px-6 md:px-8 bg-[#f8faff]">
        <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden mb-10 sm:mb-20">
          <div className="bg-gradient-to-r from-blue-900 to-blue-700 px-4 sm:px-10 py-5 sm:py-8">
            <h2 className="text-lg sm:text-2xl md:text-3xl font-bold text-white text-center">Passport Application Form</h2>
